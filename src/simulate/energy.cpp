...
#include "cells.hpp" //由唐愈涵添加，目的是实现局部场
...
namespace sim
{
    ...
    // 由唐愈涵添加，目的是实现任意添加局部场能量函数
    double spin_cell_local_field_energy(const int atom, const double Sx, const double Sy, const double Sz)
    {
        // 检查数组是否已初始化并且索引有效
        if (cells::local_field_array_x.empty() ||
            atom < 0 ||
            atom >= (int)cells::local_field_array_x.size())
        {
            return 0.0; // 如果数组未初始化或索引无效，返回0能量
        }

        double fx = cells::local_field_array_x[atom];
        double fy = cells::local_field_array_y[atom];
        double fz = cells::local_field_array_z[atom];
        double energy = -(fx * Sx + fy * Sy + fz * Sz);
        /*
            // 只打印非零局部场的原子
            if(fx != 0.0 || fy != 0.0 || fz != 0.0){
                std::cout << "[局部场能量] 原子" << atom
                          << " field=(" << fx << "," << fy << "," << fz << ")"
                          << " S=(" << Sx << "," << Sy << "," << Sz << ")"
                          << " 能量=" << energy << std::endl;
            }
    ...
    double calculate_spin_energy(const int atom)
    {
        ...
        if (err::check == true)
            std::cout << "calculate_spin_energy has been called" << std::endl;
        ...
        // 局部场能量项（唐愈涵添加，其核心代码在src/cells/里面，能够实现给指定区域添加局部场）
        double local_field_energy = 0.0;
        if (sim::local_applied_field)
        {
            local_field_energy = spin_cell_local_field_energy(atom, Sx, Sy, Sz);
            energy += local_field_energy;
        } // 调试输出：每个原子的总能量和局部场能量
        /* if(local_field_energy != 0.0){
             std::cout << "[调试] atom " << atom
                       << " 总能量=" << energy
                       << " 局部场能量=" << local_field_energy << std::endl;
         }
         //-----------------------------------------------------------------------//
     */
        ...
        if (sim::local_applied_field)
            energy += spin_local_applied_field_energy(imaterial, Sx, Sy, Sz);
