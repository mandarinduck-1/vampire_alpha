...
#include "cells.hpp" // 唐愈涵加的，目的是在进行磁滞回线模拟的时候使用局部场
...
void calculate_spin_fields(const int start_index,const int end_index){
   ...
   anisotropy::fields(atoms::x_spin_array, atoms::y_spin_array, atoms::z_spin_array, atoms::type_array,
    ...
    for(int atom=start_index; atom<end_index; ++atom){
        atoms::x_total_spin_field_array[atom] += cells::local_field_array_x[atom];
        atoms::y_total_spin_field_array[atom] += cells::local_field_array_y[atom];
        atoms::z_total_spin_field_array[atom] += cells::local_field_array_z[atom];
    }
    // ============================
