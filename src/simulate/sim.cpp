...
namespace sim
{
	...
	// 定义全局变量，用于存储检查点（checkpoint）时的状态信息。
	// parity：奇偶性标志，初始为-1。
	// output_atoms_file_counter：原子输出文件计数器。
	// output_cells_file_counter：单元格输出文件计数器。
	// output_rate_counter：输出速率计数器。

	bool ext_demag = false;
	// 表示使用外部去磁场，默认关闭（demag在多种计算函数库里都是代表去磁场）
	...
	int run()
	{
		...
		if (err::check == true)
			std::cout << "sim::run has been called" << std::endl;
...
// Set up statistical data sets
			...
			{
				if (mp::material[m].non_magnetic == 2)
					non_magnetic_materials_array[m] = true;
			}
		...
		stats::update();
		if (!load_checkpoint_continue_flag)
			stats::reset();

		// For continuous checkpoints inform user about I/O
		if (sim::save_checkpoint_continuous_flag)
			zlog << zTs() << "Continuously writing checkpoints to disk throughout simulation." << std::endl;
		...
		if (environment::enabled)
			environment::initialize(cs::system_dimensions[0], cs::system_dimensions[1], cs::system_dimensions[2]);
...
// Now set initial compute time
		...
		case 101:
			if (vmpi::my_rank == 0)
			{
				std::cout << "segmented-hysteresis-loop..." << std::endl;
				zlog << "segmented-hysteresis-loop..." << std::endl;
			}
			program::segmented_hysteresis_loop();
			break;
			//----------------------------------------------
		...
		if (sim::save_checkpoint_flag && !sim::save_checkpoint_continuous_flag)
			save_checkpoint();
	...
	int integrate(uint64_t n_steps)
	{
		...
		// std::cout << "[调试] sim::integrate() 被调用，步数参数: " << n_steps << std::endl;
		//  Check for calling of function
		if (err::check == true)
			std::cout << "sim::integrate has been called" << std::endl;
...
// Call serial or parallell depending at compile time
	...
	void integrate_serial(uint64_t n_steps)
	{
		...
		// std::cout << "[调试] sim::integrate_serial() 被调用，步数参数: " << n_steps << std::endl;

		// Check for calling of function
		if (err::check == true)
			std::cout << "sim::integrate_serial has been called" << std::endl;

		// if simulation is micromagnetic
		if (micromagnetic::discretisation_type > 0)
			multiscale_simulation_steps(n_steps);
	...
	int integrate_mpi(uint64_t n_steps)
	{
		...
		// Check for calling of function
		if (err::check == true)
			std::cout << "sim::integrate_mpi has been called" << std::endl;
	...
	for (int ti = 0; ti < n_steps; ti++)
	{
		...
		if (micromagnetic::number_of_micromagnetic_cells > 0 && (sim::time) % micromagnetic::num_atomic_steps_mm == 0)
		{
			...
			if (micromagnetic::integrator == 0)
				micromagnetic::LLG(cells::local_cell_array,
								   n_steps,
								   cells::num_cells,
								   cells::num_local_cells,
								   sim::temperature,
			...
			// if LLB run an LLB step
				...
				micromagnetic::LLB(cells::local_cell_array,
								   n_steps,
								   cells::num_cells,
								   cells::num_local_cells,
								   sim::temperature,
		...
		if (micromagnetic::number_of_atomistic_atoms > 0)
			micromagnetic::atomistic_LLG_Heun();
