...
#include "units.hpp" //唐愈涵加的，以实现分段磁滞回线
...
namespace program
{
   ...
   bool match_input_parameter(std::string const key, std::string const word, std::string const value, std::string const unit, int const line)
   {
      ...
      if (word == test)
      {
         ...
         // 为了实现分段磁滞回线由唐愈涵加入的代码
         test = "segmented-hysteresis-loop";
         if (value == test)
         {
            program::program = 101; // 你可以自定义编号
            return true;
         }
         ...
         else
         {
            ...
            // 为了代码的一致性由唐愈涵加入的
            std::cerr << "\t\"segmented-hysteresis-loop\"" << std::endl;
            //
      ...
      if (word == test)
      {
         ...
         program::internal::segmented_hyst_segments.resize(n);
         return true;
      }
      for (size_t i = 1; i <= program::internal::segmented_hyst_segments.size(); ++i)
      {
         std::string prefix = "segment-" + std::to_string(i) + "-";
         if (word == prefix + "min")
         {
            double val = atof(value.c_str());
            std::string type;
            units::convert(unit, val, type); // 支持单位
            program::internal::segmented_hyst_segments[i - 1].Hmin = val;
            return true;
         }
         if (word == prefix + "max")
         {
            ...
            std::string type;
            units::convert(unit, val, type); // 支持单位
            program::internal::segmented_hyst_segments[i - 1].Hmax = val;
            return true;
         }
         if (word == prefix + "inc")
         {
            ...
            std::string type;
            units::convert(unit, val, type); // 支持单位
            program::internal::segmented_hyst_segments[i - 1].Hinc = val;
            return true;
         }
         if (word == prefix + "equil")
         {
            program::internal::segmented_hyst_segments[i - 1].equil_steps = atoll(value.c_str());
            return true;
         }
         if (word == prefix + "loop")
         {
            program::internal::segmented_hyst_segments[i - 1].loop_steps = atoll(value.c_str());
            return true;
         }
      }
   ...
   bool match_material_parameter(std::string const word, std::string const value, std::string const unit, int const line, int const super_index, const int sub_index)
   {
      ...
      if ((unsigned int)super_index + 1 > internal::mp.size() && super_index + 1 < 101)
