         ...
         */
             ...
             cu_real_t dt, size_t num_atoms)
         {
                 ...
                 atom += blockDim.x * gridDim.x)
            {
               ...
               // ======唐愈涵加的目的是实现局部场======
               // calculate total field (添加局部场)
               cu_real_t H_x = x_sp_field[atom] + x_ext_field[atom] + local_field_x[atom];
               cu_real_t H_y = y_sp_field[atom] + y_ext_field[atom] + local_field_y[atom];
               cu_real_t H_z = z_sp_field[atom] + z_ext_field[atom] + local_field_z[atom];
               // ============================
